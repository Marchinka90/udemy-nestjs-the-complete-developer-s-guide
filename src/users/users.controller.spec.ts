...
describe('UsersController', () => {
    ...
    it('findAllUsers returns a list of users with the given email ', async () => {
        const users = await controller.findAllUsers('autotest@test.com');
        expect(users.length).toEqual(1);
        expect(users[0].email).toEqual('autotest@test.com');
    });
    ...
    it('singin updates session object and returns user ', async () => {
        ...
        const user = await controller.signin({ email: 'autotest@test.com', password: '123456' }, session);
